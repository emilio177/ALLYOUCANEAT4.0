...
";
    ...
    public function comprarCarrito($id_usuario, $id)
    {
        ...
        foreach ($carritoCursos as $curso) {
            ...
            $query = "INSERT INTO detalle_venta (id_producto, id_usuario, fecha,cantidad,subtotal,id_transaccion) VALUES (:id_producto, :id_usuario,  NOW(),:cantidad,:subtotal,:id_transaccion)";
            $rs = $this->db->prepare($query);
            $rs->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
            $rs->bindParam(':id_producto', $id_producto, PDO::PARAM_INT);
            $rs->bindParam(':cantidad', $cantidad, PDO::PARAM_INT);
            $rs->bindParam(':subtotal', $subtotal, PDO::PARAM_INT);
            $rs->bindParam(':id_transaccion', $id, PDO::PARAM_STR);
